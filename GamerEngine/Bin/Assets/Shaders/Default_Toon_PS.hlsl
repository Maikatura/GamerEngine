...
PixelOutput main(VertexToPixel input)
{
    ...
    float3 lightingSum = directLighting + ambientLighting + emissiveColor + ((pointLight + spotLight));

	// Calculate the dot product of the light direction and the normal
    float lightDotNormal = dot(normal, -LB_DirectionalLight.Direction);

	// Use the dot product to select a color from a small set of colors
    float3 finalColor;
    if (lightDotNormal > 0.95)
    {
        finalColor = saturate(lightingSum) * float3(1.0, 1.0, 1.0); // Bright color
    }
    else if (lightDotNormal > 0.5)
    {
        finalColor = saturate(lightingSum) * float3(0.8, 0.8, 0.8); // Medium color
    }
    else
    {
        finalColor = saturate(lightingSum) * float3(0.4, 0.4, 0.4); // Dark color
    }
