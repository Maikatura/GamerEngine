...
namespace TGA
{
	...
	namespace FBX
	{
		...
		unsigned int GatherMeshesAndLODGroups(FbxNode* aNode, std::vector<FbxNode*>& outMeshes, std::vector<FbxNode*>& outLODGroups)
		{
			...
			for(int i = 0; i < aNode->GetChildCount(); i++)
			{
		        ...
		        if(!childAttribute)
		        {
			        ...
			        GatherMeshesAndLODGroups(childNode, outMeshes, outLODGroups);
		        }            
		        else
		        {
			        const FbxNodeAttribute::EType childAttributeType = childAttribute->GetAttributeType();

					// Check if this is a LOD group.
					if (childAttributeType == FbxNodeAttribute::eLODGroup)
					{
						// It is, so gather it and ignore its children.
						outLODGroups.push_back(childNode);
					}
					else
					{
		...
		/**
		 * Converts a single FbxNode into a series of intermediate FBXMeshes.
		 * @param mdlMeshNode The mesh node to convert.
		 * @param fbxScene The FBX Scene we belong to.
		 * @param hasBones If the scene we are working in has a skeleton or not.
		 * @param mdlSkeleton The skeleton, if there is one.
		 * @param inOutModel The actual model we're working on that contains global model data such as material.
		 */
		std::vector<FBXModel::FBXMesh> ConvertMesh(FbxNode* mdlMeshNode, FbxScene* fbxScene, bool hasBones, FBXSkeleton mdlSkeleton, FBXModel& inOutModel)
		{
			std::vector<FBXModel::FBXMesh> outMeshes;
			...
			for (int meshMaterialIndex = 0; meshMaterialIndex < mdlMeshNode->GetMaterialCount() || meshMaterialIndex == 0; meshMaterialIndex++)
			{
				...
				if (bHasMaterials)
				{
					for (int sceneMaterialIndex = 0; sceneMaterialIndex < fbxScene->GetMaterialCount(); sceneMaterialIndex++)
					{
						...
						if (sceneMaterial == meshNodeMaterial)
						{
							...
							if (inOutModel.Materials[sceneMaterialIndex].MaterialName.empty())
							{
								// This material probably doesn't exist so we need to create it.
								FBX::GetMaterialData(sceneMaterial, inOutModel.Materials[sceneMaterialIndex]);
							}
				...
				for (int p = 0; p < fbxMeshPolygonCount; p++)
				{
					...
					for (int v = 0; v < 3; v++)
					{
						...
						if(fbxNumVxColorChannels > 1)
						{
							for (int vxColSet = 1; vxColSet < fbxNumVxColorChannels; vxColSet++)
							{
	...
	bool FBXImporter::LoadModel(const std::string& someFilePath, FBXModel& outModel)
	{
		...
		if(fbxImporter->Import(fbxScene))
		{
			...
			std::vector<FbxNode*> mdlLodGroups;
			mdlLodGroups.reserve(numSceneMaterials);

			// This might result in 0 meshes that are not part of a LOD group.
	        const unsigned int numMeshes = FBX::GatherMeshesAndLODGroups(fbxScene->GetRootNode(), mdlMeshNodes, mdlLodGroups);
			...
			// Generate data in all lod group meshes too if they exist.
			for(FbxNode* mdlMeshNode : mdlLodGroups)
			{
				if(FbxMesh* fbxMesh = mdlMeshNode->GetMesh())
				{
					if (fbxMesh->GetElementBinormalCount() == 0 || fbxMesh->GetElementTangentCount() == 0)
					{
						if (!fbxMesh->GenerateTangentsData(0, true, false))
			...
			if(!bImporterError)
			{
				...
				std::vector<FBXModel::FBXLODGroup> mdlLODGroups;
				mdlMeshData.reserve(numMeshes * numSceneMaterials);
				mdlLODGroups.reserve(numMeshes * numSceneMaterials * mdlLodGroups.size());

				// First we crawl through all meshes that are not part of a LOD group.
				for(FbxNode* mdlMeshNode : mdlMeshNodes)
				{
					const std::vector<FBXModel::FBXMesh> meshes = FBX::ConvertMesh(mdlMeshNode, fbxScene, hasBones, mdlSkeleton, outModel);
					mdlMeshData.insert(mdlMeshData.end(), meshes.begin(), meshes.end());
				}

				// Then we crawl through all LOD grouped meshes.
				for(FbxNode* mdlLodNode : mdlLodGroups)
				{
					FBXModel::FBXLODGroup lodGroup;
					FbxLODGroup* fbxLodGroupNode = mdlLodNode->GetLodGroup();

					const int numLodLevels = mdlLodNode->GetChildCount();
					for (int lodLv = 0; lodLv < numLodLevels; lodLv++)
					{
						FbxDistance aDistance;
						if(lodLv != 0)
						{
							// If this returns True and the file is set to use LOD threshold as Distance we get the distance.
							// If the file is set to use LOD threshold as Percentage it will return false :P. But only if ThresholdsUsedAsPercentage is true :P.
							if (fbxLodGroupNode->GetThreshold(lodLv - 1, aDistance))
							{}
						}

						FbxNode* lodLvRoot = mdlLodNode->GetChild(lodLv);
						FBXModel::FBXLODGroup::FBXLodLevel currentLevel;
						currentLevel.Level = lodLv;
						currentLevel.Distance = aDistance.value();

						std::vector<FbxNode*> lodGroupMeshNodes;
						std::vector<FbxNode*> lodGroupGroups; // Not used;
						const unsigned numLodLevelMeshes = FBX::GatherMeshesAndLODGroups(lodLvRoot, lodGroupMeshNodes, lodGroupGroups);

						currentLevel.Meshes.reserve(numLodLevelMeshes);
						for (FbxNode* mdlLodMesh : lodGroupMeshNodes)
						{
							const std::vector<FBXModel::FBXMesh> lodGroupMeshes = FBX::ConvertMesh(mdlLodMesh, fbxScene, hasBones, mdlSkeleton, outModel);
							currentLevel.Meshes.insert(currentLevel.Meshes.end(), lodGroupMeshes.begin(), lodGroupMeshes.end());
						}

						lodGroup.Levels.push_back(currentLevel);
					}

					mdlLODGroups.push_back(lodGroup);
				}

				if(mdlMeshData.empty() && mdlLodGroups.empty())
				{
	...
	bool FBXImporter::LoadNavMesh(const std::string& someFilePath, FBXNavMesh& outNavMesh, bool shouldTriangulate)
	{
		FbxImporter* fbxImporter = FbxImporter::Create(Internals::Get().fbxManager, "");
		...
		if (fbxImporter->Import(fbxScene))
		{
			...
			std::vector<FbxNode*> mdlLODGroups;
			const unsigned int numMeshes = FBX::GatherMeshesAndLODGroups(fbxScene->GetRootNode(), mdlMeshNodes, mdlLODGroups);
			mdlMeshNodes.reserve(numMeshes);

			float maxExtents[3] = { 0, 0, 0 };
			float minExtents[3] = { 0, 0, 0 };

			// Technically we can generate normals here if we want to and Tangents/Binormals too
			//for (FbxNode* mdlMeshNode : mdlMeshNodes)
			//{
			//	FbxMesh* fbxMesh = mdlMeshNode->GetMesh();
			...
			if (!bImporterError)
			{
				...
				std::vector<FBXNavMesh::FBXNavMeshChunk> navMeshChunks;
				navMeshChunks.reserve(numMeshes);

				for (FbxNode* mdlMeshNode : mdlMeshNodes)
				{
					FbxMesh* fbxMesh = mdlMeshNode->GetMesh();

					FBXNavMesh::FBXNavMeshChunk chunk;
	...
	bool FBXImporter::LoadAnimation(const std::string& someFilePath, FBXAnimation& outAnimation)
	{
		...
		if(fbxImporter->Import(fbxScene))
		{
			...
			// Look for a skeleton in this scene.
			FBXSkeleton mdlSkeleton;
			bool hasBones = false;
			std::string gatherBonesMessage;
			if(FBX::GatherSkeletonData(fbxScene->GetRootNode(), mdlSkeleton, gatherBonesMessage) != 0)
			{
				myLastError = gatherBonesMessage;
				bImporterError = true;
			}
			else
			{
				...
				const ::FbxNode* skeletonRoot = fbxScene->FindNodeByName(FbxString(mdlSkeleton.Bones[0].Name.c_str()));
					...
					if (const FbxAnimStack* fbxAnimStack = fbxScene->GetSrcObject<FbxAnimStack>(0))
					{
						...
						if (const FbxTakeInfo* fbxTakeInfo = fbxScene->GetTakeInfo(animStackName))
						{
							...
							for (FbxLongLong t = startFrame; t <= endFrame && !bImporterError; t++)
							{
								...
								for (size_t j = 0; j < mdlSkeleton.Bones.size() && !bImporterError; j++)
								{
									...
									if (::FbxNode* jointNode = fbxScene->FindNodeByName(FbxString(mdlSkeleton.Bones[j].Name.c_str())))
									{
									...
									FBX::ConvertFBXMatrixToArray(localFrameTransform, LocalTransform.Data);
									FBX::ConvertFBXMatrixToArray(globalFrameTransform, GlobalTransform.Data);

									outAnimation.Frames[localFrameCounter].LocalTransforms.insert({ mdlSkeleton.Bones[j].Name, LocalTransform });
									outAnimation.Frames[localFrameCounter].GlobalTransforms.insert({ mdlSkeleton.Bones[j].Name, GlobalTransform });
								}
