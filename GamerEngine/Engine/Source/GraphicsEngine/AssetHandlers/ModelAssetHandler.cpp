		...
		for (size_t i = 0; i < tgaModel.Meshes.size(); i++)
		{
			...
			for(size_t v = 0; v < mesh.Vertices.size(); v++)
			{
				...
				mdlVertices[v].VertexColors[0] = {
					Helpers::RandomFlaotRange(0.0f, 1.0f),
					Helpers::RandomFlaotRange(0.0f, 1.0f),
					Helpers::RandomFlaotRange(0.0f, 1.0f),
					1.0f
				};
...
bool ModelAssetHandler::LoadAnimation(const std::wstring& aModelName, const std::wstring& someFilePath) 
{
	...
	{
		Animation animOut;
		animOut.Duration = tgaAnimation.Duration;
		...
		for(int i = 0; i < tgaAnimation.Length; i++)
		{
			animOut.Frames.push_back(Animation::Frame{ *(std::vector<CommonUtilities::Matrix4x4<float>>*) & tgaAnimation.Frames[i].LocalTransforms });
		}
...
std::shared_ptr<ModelInstance> ModelAssetHandler::GetModelInstance(const std::wstring& aFilePath) 
{
	...
	modelInstance->Init(myModelRegistry[aFilePath]);
