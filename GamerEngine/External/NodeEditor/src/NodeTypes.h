...
struct DataPtr
{
	...
	static DataPtr Create(DataType aType, size_t someSize = 1)
	{
		...
		case DataType::Vector3:
			result.Ptr = malloc(sizeof(float) * 3);
			::new (result.Ptr) float[3]();
			result.Size = sizeof(float) * 3;
			break;

		case DataType::Vector2:
			result.Ptr = malloc(sizeof(float) * 2);
			::new (result.Ptr) float[2];
			result.Size = sizeof(float) * 2;
			break;
