...
bool TurNet::ClientUDP::Start()
{
	...
	ioctlsocket(myConnectSocket, FIONBIO, &mode);
...
void TurNet::ClientUDP::StartWorker()
{
	...
	myWorkerThread.reset(new std::thread([this]() { WorkerThread(); }));
}
...
int TurNet::ClientUDP::SendToServer(TurNet::TurMessage& aMessage)
{
	...
	std::memcpy(aDataBuffer, crypted.c_str(), crypted.length());
...
void TurNet::ClientUDP::WorkerThread()
{
	while(myWorkerShouldRun)
	{
		...
		nRet = recvfrom(myConnectSocket, messageData.Buffer, messageData.BufferLength, 0, (struct sockaddr*)&messageData.Connection, &slen);
		if(nRet == 0)
		{
