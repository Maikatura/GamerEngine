...
void TurNet::ServerUDP::WorkerThread()
{
	...
	while(myWorkerShouldRun)
	{
		try
		{
			...
			nRet = recvfrom(myListenSocket, messageData.Buffer, messageData.BufferLength, 0, (struct sockaddr*)&messageData.Connection, &slen);
			if(nRet == 0)
			{
			...
			else
			{
				...
				sockaddr_in* sin = reinterpret_cast<sockaddr_in*>(&messageData.Connection);
