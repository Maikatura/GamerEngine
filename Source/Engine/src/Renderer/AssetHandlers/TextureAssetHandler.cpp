...
std::shared_ptr<Texture> TextureAssetHandler::GetTexture(const std::wstring& aName)
{
	...
	if(model == myRegistry.end())
	{
		LoadTexture(L"Editor\\Textures\\T_Default_C.dds");
		return myRegistry.find(L"Editor\\Textures\\T_Default_C.dds")->second;
	}
...
std::unique_ptr<DepthStencil> TextureAssetHandler::CreateDepthStencil(
