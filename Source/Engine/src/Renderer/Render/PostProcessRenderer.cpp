...
PostProcessRenderer::RenderTexture PostProcessRenderer::CreateRenderTexture(const std::string& aName, float aWidth, float aHeight, DXGI_FORMAT aFormat)
{
	if(auto i = myRenderTextures.find(aName); i == myRenderTextures.end())
	{
		...
		depthBufferDesc.Width = static_cast<UINT>(aWidth);
		depthBufferDesc.Height = static_cast<UINT>(aHeight);
		depthBufferDesc.ArraySize = 1;
		depthBufferDesc.Format = DXGI_FORMAT_D32_FLOAT;
		depthBufferDesc.SampleDesc.Count = 1;
		depthBufferDesc.BindFlags = D3D11_BIND_DEPTH_STENCIL;

		DX11::Device->CreateTexture2D(&depthBufferDesc, nullptr, renderTex.myTexture2D.GetAddressOf());
		DX11::Device->CreateDepthStencilView(renderTex.myTexture2D.Get(), nullptr, renderTex.myDSV.GetAddressOf());
		...
		DX11::Device->CreateTexture2D(&bufferDesc, nullptr, renderTex.myTexture2D.GetAddressOf());
		DX11::Device->CreateRenderTargetView(renderTex.myTexture2D.Get(), nullptr, renderTex.myRTV.GetAddressOf());
		DX11::Device->CreateShaderResourceView(renderTex.myTexture2D.Get(), nullptr, renderTex.mySRV.GetAddressOf());
		//SafeRelease(renderTex.myTexture2D);
...
void PostProcessRenderer::Render(PostProcessPass aPass)
{
	...
	switch(aPass)
	{
		...
		case PP_SSAO:
		{
			...
			DX11::Context->VSSetShaderResources(6, 1, &nullsrv);
